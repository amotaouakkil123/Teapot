...
void main(void) {
	...
	if ( enableLighting ) {
        ...
        vec3 nonHCoors = positionLightCVV.xyz/positionLightCVV.w;
        nonHCoors = nonHCoors * 0.5 + 0.5;
        float distanceFromLight = texture(depthMap, nonHCoors.xy).r;
        float depth = nonHCoors.z;
        float bias = sigma * tan(acos(diffuse));
        bias = clamp(bias, 0, sigma);
        if ( depth - bias > distanceFromLight)
        {
            shadow = 0.0f;
        }
